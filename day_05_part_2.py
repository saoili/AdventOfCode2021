...
class Ocean:
  ...
  def add_diagonal_line_segment(self, x1, y1, x2, y2):

    # print(f'adding diagonal line from {x1}, {y1} to {x2}, {y2}')
    if x1 > x2:
      x1, x2 = x2, x1
      y1, y2 = y2, y1
    # print(f'adding diagonal line from {x1}, {y1} to {x2}, {y2}')
    ...
    for this_x in range(x1, x2+1):
      # print(f'adding 1 a {this_x, this_y}')
  ...
  def add_line_segment(self, x1, y1, x2, y2):
    if x1 != x2 and y1 != y2:
      self.add_diagonal_line_segment(x1, y1, x2, y2)
      return
